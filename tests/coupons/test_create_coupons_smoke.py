...
def test_create_coupon_percent_discount_type(my_setup, discount_type):
    ...
    assert rs_coupon_2['code'] == coupon_code.lower(), \
        f"Create coupon response has wrong 'code'. " \
        f"Expected: {coupon_code.lower()}, Actual: {rs_coupon_2['code']}."
    assert rs_coupon_2['discount_type'] == expected_discount_type, \
        f"Create coupon responded with wrong 'discount_type'." \
...
def test_create_coupon_with_invalid_discount_type():
    ...
    assert rs_coupon['code'] == 'rest_invalid_param', \
        f"Crete coupon with invalid 'discount_type' " \
        f"returned 'code={rs_coupon['code']}', Expected code = 'rest_invalid_param' "
    assert rs_coupon['message'] == 'Invalid parameter(s): discount_type', \
        f"Crete coupon with invalid 'discount_type' " \
